...
export function getMovies(
  db: Database,
  filters: { title: string | undefined; genres: string | undefined }
) {
  const { title, genres } = filters
  let movies

  if (title && genres) {
    movies = getAllMoviesByTitleAndGenre(db, title, genres)
  } else if (title) {
    movies = getAllMoviesByTitle(db, title)
  } else if (genres) {
    movies = getAllMoviesByGenre(db, genres)
  } else {
...
export function getAllMoviesByTitleAndGenre(
  db: Database,
  title: string,
  genres: string
) {
  const query = db.query(
    `SELECT * FROM movies WHERE title LIKE ? AND genres LIKE ?`
  )
